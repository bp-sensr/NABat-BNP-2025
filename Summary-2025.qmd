...
# FORCE_FAMILY <- c(EPFU = "NB2", '40KMyo' = "NB2", LACI = "NB2")
# # ----------------------------- HELPERS -------------------------------
...
# dat_raw <- read.csv("Data/NABat_Banff2020-2025-detector.csv")
# dat_raw <- dat_raw %>%
#   mutate(
#     date    = as.Date(date),
...
#       theme_classic()
# 
#     # 40KMyo's CI ribbon is so wide it visually flattens the trend line;
#     # clip the y-axis display to 100 for this species only.
#     # Use coord_cartesian (not ylim/scale_y_continuous) so the ribbon and
#     # line are drawn over the full data and only clipped visually — no rows dropped.
#     if (SP == "40KMyo") {
...
#----------Trends summary values-------------------------
# YearS trend on the IRR (response) scale — computed identically to the
# gt table so the prose and @tbl-spptrend always agree.
irr_vals <- models_summary |>
  transmute(
    sp,
    irr  = exp(YearS_estimate),
    low  = exp(YearS_estimate - 1.96 * YearS_se),
    high = exp(YearS_estimate + 1.96 * YearS_se),
    p    = YearS_p
  )

# Pull exactly one species row; error loudly on a code/typo mismatch
sp_row <- function(code) {
  r <- irr_vals[irr_vals$sp == code, ]
  if (nrow(r) != 1L) stop("Expected one row for '", code, "', got ", nrow(r))
  r
}
...
irr  <- function(code) sprintf("%.2f", sp_row(code)$irr)
ci   <- function(code) { r <- sp_row(code); sprintf("%.2f\u2013%.2f", r$low, r$high) }
pval <- function(code) fmt_p(sp_row(code)$p)
pct  <- function(code) round(abs(1 - sp_row(code)$irr) * 100)

irr_ci_p <- function(code)
  sprintf("IRR = %s, 95%% CI %s; p = %s", irr(code), ci(code), pval(code))

min_p <- sprintf("%.2f", min(irr_vals$p))
...
Banff biologists deployed three stationary bat detectors in strategically selected bat habitat across distinct quadrants within the NABat grid cell (GRID Cell ID: 148842; @fig-sites). Monitoring began at the Fenlands site in 2020, with the Golf Course and Upper Hotsprings sites added in 2021; all three sites were then surveyed consistently each year through 2025. Detectors set out between June 24--30 and retrieved between July 2--23 ( @tbl-deployments). The comparable deployment windows across years allow for interannual comparison. Deployment setup followed the standards established by @loeb2015Plan. Titley Swift units were used in most years, except in 2020 and 2024, when Wildlife Acoustics SM4 units were deployed instead.
...
#| message: false
#| include: true
#| tbl-cap: Detector deployments by survey location in the Banff NABat grid cell, 2020–2025, showing deployment year, detector unit, start and end dates, and number of survey nights.
#| tbl-pos: H
...
deploy_tbl <- sites |>
  ...
  transmute(
    ...
    End    = format(end,   "%d %b"),
    Nights = if_else(transect, 1L, as.integer(end - start))
  )
...
#| include: true
#| tbl-pos: H
#| tbl-cap: Year-over-year trend estimates for six bat taxa in the Banff NABat grid cell, 2020–2025. Trends are from negative binomial GLMMs of nightly detections and are expressed as incidence rate ratios (IRRs) for the year term. IRR > 1 indicates an increasing trend, IRR < 1 a decreasing trend, and IRR = 1 no change. A 95% confidence interval spanning 1.0 indicates the trend is not statistically significant at α = 0.05. "Covariates Used" lists the environmental terms retained in each taxon's model; a deployment random intercept, year, and detector unit were included in all models.
#| label: tbl-spptrend
...
trend_tbl <- models_summary |>
  ...
  transmute(
    ...
    irr_ci = sprintf(
      "%.2f (%.2f\u2013%.2f)",
      exp(YearS_estimate),
      exp(YearS_estimate - 1.96 * YearS_se),
      exp(YearS_estimate + 1.96 * YearS_se)
    ),
    p_num   = YearS_p,                                    # numeric, for the bolding logic
...
trends <- trend_tbl |>
  ...
  cols_hide(columns = p_num) |>         # keep it in the data, off the table
  cols_align(align = "left",   columns = c(Species, Covariates)) |>
  cols_align(align = "center", columns = c(irr_ci, p_value)) |>
  #adjust col widths
  cols_width(
    Species    ~ px(80),
    Covariates ~ px(470),   # tightened — long covariate lists wrap to 2 lines
    irr_ci     ~ px(120),   # widened
    p_value    ~ px(70)
  ) |>
  tab_style(
    style     = cell_text(weight = "bold"),
  ...
  tab_style(
    style     = cell_text(weight = "bold"),
    locations = cells_body(rows = p_num < 0.05) 
  ) |>
  opt_css(css = "
    #tbl-trends, #tbl-trends table.gt_table {
      table-layout: fixed !important;
      width: 740px !important;
    }
    #tbl-trends .gt_table td, #tbl-trends .gt_table th {
...
Estimates varied in both direction and magnitude. Big Brown Bat (EPFU; `r irr_ci_p("EPFU")`) and Long-eared Myotis (MYEV; `r irr_ci_p("MYEV")`) trended positive, while Eastern Red Bat (LABO; `r irr_ci_p("LABO")`) and Silver-haired Bat (LANO; `r irr_ci_p("LANO")`) were essentially flat to weakly negative (@fig-trends). None of these approached significance.

Two taxa showed negative estimates that, while not significant, are important to note. The 40 kHz Myotis complex (40KMyo) had the steepest negative trend, with an IRR of `r irr("40KMyo")` (95% CI `r ci("40KMyo")`; *p* = `r pval("40KMyo")`). This estimate corresponds to roughly a `r pct("40KMyo")`% decline in nightly activity per year. The Hoary Bat likewise trended negative (LACI; `r irr_ci_p("LACI")`), corresponding to an estimate of about a `r pct("LACI")`% annual decline.

![Species relative activity trends from 2020--2025 in the Banff NABat grid cell. Lines show trend estimates of nightly detections with 95% confidence intervals. Dashed lines indicate non-significant trends (p \> 0.05). Models are based on nightly acoustic detections and fitted using negative binomial mixed-effects models.](Figures/Trends/bat_trends_multipanel.png){#fig-trends fig-pos="H"}

# Discussion

Our six-year activity-trend analysis for the Banff grid cell revealed no statistically significant directional change in nightly bat activity for any of the six modelled taxa, providing preliminary evidence of relatively stable bat activity within the cell since monitoring began in 2020. Four taxa trended positive or flat: Big Brown Bat (EPFU) and Long-eared Myotis (MYEV) positive, Silver-haired Bat (LANO) and Eastern Red Bat (LABO) essentially flat ( @fig-trends). Two taxa showed negative point estimates: the 40 kHz Myotis complex (40KMyo) declined by roughly 30% per year and Hoary bat (LACI) by roughly 10% per year. Although neither reached significance, both are taxa of conservation concern, and continued monitoring will be needed to determine whether these apparent declines are real.

These local patterns are best interpreted alongside broader-scale analyses. Province-wide NABat relative abundance modelling has documented a significant \~40% decline in abundance for Hoary Bats across Alberta between 2012 and 2023 (mean = −0.39, 95% CI −0.61 to −0.13), together with a smaller, non-significant decline of \~15% for Silver-haired bats (mean = −0.15, 95% CI −0.45 to 0.22; [@bradleyjudell]). Although our grid-cell estimates were not significant, the negative direction of our Hoary bat trend is consistent with this regional signal. The difference in statistical power is expected: the provincial estimates integrate data across all of Alberta, whereas our analysis is restricted to a single grid cell. Our smaller sample size and spatial extent produce greater variance and reduced power to detect trends, so the absence of local significance does not contradict the broader decline. Placing our results within this regional and continental context is therefore essential to their interpretation. The decline of migratory tree bats such as the Hoary Bat has been widely linked to mortality at wind-energy facilities [@zimmerling2016Bat]. The continued expansion of wind-energy development across the region may therefore place further pressure on this species.

The 40KMyo complex warrants particular attention. NABat occupancy analyses for Alberta (2010–2019) suggest that Little Brown bat, Northern Myotis, and Long-legged Myotis populations were stable over that period [@udell2022]. Our complex, which we expect is dominated by the Little Brown bat, instead points to a potential decline, albeit without statistical significance. This divergence is most likely due to the recent arrival of white-nose syndrome (WNS) to the province. *Pseudogymnoascus destructans* (Pd), the causative fungus, was first detected in Alberta in 2022 and has since spread across the province [@cwhc-rcsf2024Canadian]. WNS was confirmed in the south eastern part of the province in 2023, and as of winter 2026 WNS has been confirmed in the Rocky Mountains, including at Cadomin Cave, a major regional hibernaculum (L. Wilkinson, pers. comm.). Given the severe overwinter mortality WNS causes in hibernating *Myotis*, marked declines inLittle Brown bat and Northern Myotis, are anticipated in the coming years. While our study period largely predates confirmed WNS at this location, the negative 40KMyo estimate underscores the importance of sustained monitoring of this complex as the disease establishes.
...
Overall, and with these caveats in mind, our results indicate that bat activity within the Banff grid cell has remained relatively stable over the 2020–2025 monitoring period. However, the negative point estimates for Hoary bat and 40KMyotis comples, combined with documented regional declines and the ongoing spread of WNS and wind turbine development, reinforce the value of continued long-term acoustic monitoring at this site.
