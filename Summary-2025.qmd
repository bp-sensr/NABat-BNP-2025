...
Two taxa showed negative estimates that, while not significant, are important to note. The 40 kHz Myotis complex (40KMyo) had the steepest negative trend, with an IRR of `r irr("40KMyo")` (95% CI `r ci("40KMyo")`; *p* = `r pval("40KMyo")`). This estimate corresponds to roughly a `r pct("40KMyo")`% decline in nightly activity per year. The Hoary Bat likewise trended negative (LACI; `r irr_ci_p("LACI")`), corresponding to a point estimate of about a `r pct("LACI")`% annual decline.
...
trends <- trend_tbl |>
  ...
  cols_width(
    Species    ~ px(80),
    Covariates ~ px(150),   # tightened — long covariate lists wrap to 2 lines
    irr_ci     ~ px(210),   # widened
    p_value    ~ px(80)
  ) |>
  tab_style(
    style     = cell_text(weight = "bold"),
  ...
  opt_css(css = "
    #tbl-trends { table-layout: fixed; width: 520px; }
    #tbl-trends td, #tbl-trends th { overflow-wrap: break-word; }
  ") |>
  opt_row_striping() |>
  tab_options(
    table.font.size            = px(15),
