...
#---Load the data-----------------------------------------------------------
df <- read.csv("Data/Final_Dataset_banff_site_night_species_with_covariates.csv")
df <- df |>
...
#This is for reference only. The report does not use this code to create anything. Jsut wanted to keep it here so people could see how the model selection happened for each species
...
# ------------------------- GLOBAL SETTINGS ---------------------------
YEARS_TO_USE <- c(2020, 2022, 2023, 2024, 2025)
...
myo_codes <- c("MYVO", "MYCA", "MYLU", "MYSE") #technically MYCA should not be in the area, but processing in previous years included it. Collapsing it into 40KMyo
...
covariates_used <- tibble(
  ...
  covariates = vapply(results[ok_flags], function(x) {
    v <- sub("^z_", "", x$support_vars)          # drop the z_ prefix for readability
...
if (length(plot_list) > 0) {
  multi <- patchwork::wrap_plots(plot_list, ncol = 3) +
    plot_annotation(title = "Banff bat trends")
